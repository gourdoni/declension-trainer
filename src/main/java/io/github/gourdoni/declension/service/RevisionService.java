...
public final class RevisionService {
    ...
    public List<RevisionResult> recordRevisions(List<RevisionResponse.Response> responses, LocalDate revisedOn) {
        return responses
                .stream()
                .map(response -> new RevisionResult(response.inflectionID(),
                                                    processRevision(response.inflectionID(), response.recall(), revisedOn).dueDate()))
                .toList();
    }
}